...
module.exports = () => {
	...
	controller.getOperators = async(req, res) => { 
        const request_origin = req.get('origin'); res.setHeader('Access-Control-Allow-Origin', request_origin); const result = await bd.selectAllOperators(); res.status(200).json(result); console.log(result); };
